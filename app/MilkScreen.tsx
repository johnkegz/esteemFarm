...
const MilkEntryScreen: React.FC = () => {
  ...
  const handleSave = () => {
    Alert.alert("Milk Entry Saved", `Animal: ${animal}\nMilk Value: ${milkValue}\nDate: ${date}`);
    // Insert save logic here, like an API call or data storage
  };

  return (
    <View style={styles.container}>
      <ThemedText style={styles.title}>Milk Entry</ThemedText>
      <InputCard label="Animal" value={animal} onChangeText={setAnimal} placeholder="Enter animal name or ID" />
      <InputCard label="Milk Value" value={milkValue} onChangeText={setMilkValue} placeholder="Enter milk amount (e.g., liters)" keyboardType="numeric" />
